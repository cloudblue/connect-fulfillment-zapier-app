...
const getParametersByPhase = async (client, productId, phase) => {
  const inventory = new Inventory(client);
  const query = {
...
const getOrderingParameters = async (client, productId) => getParametersByPhase(client, productId, 'ordering');
...
const getOrderingAndFulfillmentParameters = async (client, requestId) => {
  ...
  return getParametersByPhase(client, productId, ['ordering', 'fulfillment']);
};
