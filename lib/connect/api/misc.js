...
const getParametersByPhaseAndScope = async (client, productId, phase, scope) => {
  const inventory = new Inventory(client);
  const query = {
    phase: Array.isArray(phase) ? { $in: phase } : phase,
    scope: Array.isArray(scope) ? { $in: scope } : scope,
    $or: [
      {
...
const getProductIdByRequestId = async (client, requestId) => {
  const ff = new Fulfillment(client);
  const request = await ff.getRequest(requestId);
  return Promise.resolve(request.asset.product.id);
};

const getProductIdByTierConfigurationId = async (client, tierConfigId) => {
  const config = await client.tierConfigs.get(tierConfigId);
  return Promise.resolve(config.product.id);
};

const getProductIdByTCRId = async (client, requestId) => {
  const ff = new Fulfillment(client);
  const request = await ff.getTierConfigRequest(requestId);
  return Promise.resolve(request.configuration.product.id);
};

const getOrderingParameters = async (client, productId) => getParametersByPhaseAndScope(client, productId, 'ordering', 'asset');


const getTCRParametersByConfigId = async (client, configId) => {
  const productId = await getProductIdByTierConfigurationId(client, configId);
  if (!productId) {
    return Promise.resolve([]);
  }
  return getParametersByPhaseAndScope(client, productId, ['ordering', 'fulfillment'], ['tier1', 'tier2']);
};

const getTCRParametersByTCRId = async (client, requestId) => {
  const productId = await getProductIdByTCRId(client, requestId);
  if (!productId) {
    return Promise.resolve([]);
  }
  return getParametersByPhaseAndScope(client, productId, ['ordering', 'fulfillment'], ['tier1', 'tier2']);
...
const getOrderingAndFulfillmentParameters = async (client, requestId) => {
  const productId = await getProductIdByRequestId(client, requestId);
  if (!productId) {
    return Promise.resolve([]);
  }
  return getParametersByPhaseAndScope(client, productId, ['ordering', 'fulfillment'], 'asset');
};


const getProductParameters = async (client, productId, parameterIds) => {
  const inventory = new Inventory(client);
  const params = await inventory.searchProductParameters(productId, {
    id: { $in: parameterIds },
  });
