...
const searchListingRequests = async (client, data) => {
  ...
  do {
    ...
    if (data.status && data.status.length === 1) {
      ...
      queryParams.state = data.status[0];
    } else if (data.status && data.status.length > 1) {
      queryParams.state__in = data.status.join(',');
    }
    if (data.type && data.type.length === 1) {
      // eslint-disable-next-line prefer-destructuring
      queryParams.type = data.type[0];
    } else if (data.type && data.type.length > 1) {
      queryParams.type__in = data.type.join(',');
    }
