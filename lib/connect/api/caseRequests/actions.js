...
const searchCases = async (client, data, orderBy) => {
...
const searchCaseComments = async (client, data, orderBy) => {
  const limit = data.records_per_page || 100;
  let offset = 0;
  let results = [];
  let response = [];
...
const pendCaseRequest = async (client, data) => {
  const response = await client.cases.pend(data.case_id);
  return Promise.resolve(response);
};

const inquireCaseRequest = async (client, data) => {
  const response = await client.cases.inquire(data.case_id);
  return Promise.resolve(response);
};

const resolveCaseRequest = async (client, data) => {
  const response = await client.cases.resolve(data.case_id);
  return Promise.resolve(response);
};

const closeCaseRequest = async (client, data) => {
  const response = await client.cases.close(data.case_id);
  return Promise.resolve(response);
};
