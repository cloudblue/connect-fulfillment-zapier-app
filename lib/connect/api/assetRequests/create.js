...
const createAssetAdjustmentRequest = async (client, inputData) => {
  let req = {};
  const data = _.cloneDeep(inputData);
  const productId = getProductIdFromItems(inputData.items);
  const connectionId = await lookupConnectionByProductHub(client, productId, inputData.hub_id);
  ...
  req = buildAdjustmentRequest(data);
  req.asset.connection.id = connectionId;
...
const createAssetRequestFromOrder = async (client, data) => {
  if (!data.customer_external_uid) {
    /* eslint-disable no-param-reassign */
    data.customer_external_uid = await lookupTierByExternalId(
      client,
  ...
  if (!data.t1_external_uid) {
    /* eslint-disable no-param-reassign */
    data.t1_external_uid = await lookupTierByExternalId(
      client,
      data.t1_external_id,
      data.hub_id,
    );
  ...
  if (data.reseller_tiers === 't2t1' && !data.t2_external_uid) {
    /* eslint-disable no-param-reassign */
    data.t2_external_uid = await lookupTierByExternalId(
      client,
      data.t2_external_id,
      data.hub_id,
    );
