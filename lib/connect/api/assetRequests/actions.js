...
const FILTERS_MAP = {
  ...
  marketplace_id: 'asset.marketplace.id',
  marketplace_name: 'asset.marketplace.name',
...
const scheduleRequest = async (client, data) => {
  const plannedDate = moment(data.planned_date).toISOString();
  const response = await client.requests.schedule(
    data.request_id,
    plannedDate,
  );
  return Promise.resolve(response);
...
const revokeRequest = async (client, data) => {
  const response = await client.requests.revoke(
    data.request_id,
    data.reason,
  );
  return Promise.resolve(response);
...
const confirmRequest = async (client, data) => {
  const response = await client.requests.confirm(
    data.request_id,
  );
  return Promise.resolve(response);
};
